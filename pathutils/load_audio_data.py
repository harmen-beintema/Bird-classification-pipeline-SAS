...
def load_audio_data(base_path: str | bytes | PathLike ='./Data') -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    input:
        base_path : str | bytes | PathLike
            the base path from which to load the data from
    output: 
        raw_data : DataFrame
    ...
    if '.DS_Store' in classes:          # ignore .DS_Store folder indexing file native to macOS
    ...
    for path in classes:
        ...
        if os.path.isdir(full_path):
            ...
            for sample in range(len(dir)):     # iterate through samples in class folder
